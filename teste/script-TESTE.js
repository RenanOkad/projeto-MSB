...
// Função para verificar se o horário atual está dentro dos períodos permitidos
function isOperatingHours() {
    ...
    // Horários permitidos (em minutos desde meia-noite):
    // 9:00 às 12:00 (540 a 720 minutos)
    // 16:00 às 18:00 (960 a 1080 minutos)
    // 21:00 às 22:00 (1260 a 1320 minutos)
    return (
        (currentTime >= 540 && currentTime < 720) || // 9:00 às 12:00
        (currentTime >= 960 && currentTime < 1080) || // 16:00 às 18:00
        (currentTime >= 1260 && currentTime < 1320) // 21:00 às 22:00
    );
}

// Função para calcular o tempo até o próximo período de operação
function timeUntilNextOperatingPeriod() {
    const now = new Date();
    const hours = now.getHours();
    const minutes = now.getMinutes();
    const currentTime = hours * 60 + minutes; // Tempo em minutos desde meia-noite

    // Horários de início dos períodos (em minutos desde meia-noite)
    const periods = [
        540,  // 9:00
        960,  // 16:00
        1260  // 21:00
    ...
    // Encontra o próximo período
    let nextPeriod = periods.find(period => currentTime < period) || periods[0] + 1440; // Se passar de 22:00, volta para 9:00 do dia seguinte
    const minutesUntilNext = (nextPeriod - currentTime + 1440) % 1440; // Diferença em minutos
    return minutesUntilNext * 60 * 1000; // Converte para milissegundos
}
...
async function mainLoop() {
    ...
    let wasOperating = false; // Flag para rastrear o estado anterior (operando ou pausado)
    ...
    // Agendamento para enviar mensagem de "Iniciando o envio de sinais" nos horários de início
    schedule.scheduleJob('0 9 * * *', () => sendSystemStatus(bot, "🚀 Iniciando o envio de sinais!")); // 9:00
    schedule.scheduleJob('0 16 * * *', () => sendSystemStatus(bot, "🚀 Iniciando o envio de sinais!")); // 16:00
    schedule.scheduleJob('0 21 * * *', () => sendSystemStatus(bot, "🚀 Iniciando o envio de sinais!")); // 21:00

    // Agendamento para enviar mensagem de "Envio de sinais pausado" nos horários de término
    schedule.scheduleJob('0 12 * * *', () => sendSystemStatus(bot, "⏸️ Sessão de envio de sinais finalizados. Voltaremos no próximo horário!")); // 12:00
    schedule.scheduleJob('0 18 * * *', () => sendSystemStatus(bot, "⏸️ Sessão de envio de sinais finalizados. Voltaremos no próximo horário!")); // 18:00
    schedule.scheduleJob('0 22 * * *', () => sendSystemStatus(bot, "⏸️ Sessão de envio de sinais finalizados. Voltaremos no próximo horário!")); // 22:00
    ...
    while (true) {
        try {
            ...
            if (!isOperating) {
                if (wasOperating) {
                    // Se estava operando e agora não está, avisa que os sinais foram pausados
                    // await sendSystemStatus(bot, "⏸️ Sessão de envio de sinais finalizados. Voltaremos no próximo horário!");
                    wasOperating = false;
                }
                logger.info("Fora do horário de operação. Aguardando próximo período...");
                const waitTime = timeUntilNextOperatingPeriod();
                await delay(waitTime);
                continue;
            }

            if (!wasOperating) {
                // Se não estava operando e agora está, avisa que os sinais iniciaram
                // A mensagem já é enviada pelo agendador, mas garantimos que o estado seja atualizado
                wasOperating = true;
            }
